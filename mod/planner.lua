...
function add_construction_plan(mod_context)
    ...
    if mod_context.toolbox.power_pole ~= nil then
        plan_power(mod_context)
    end
end
------------
-- Electricity
...
function plan_power(mod_context)
    ...
    xy.each(mod_context.construction_plan, function(planned_entity, position)
        ...
        if (planned_entity.name == "extractor") then
            xy.set(consumer_positions, position, {placeholder=1--[[Maybe add some info later?]]})
        end
    end)   
...
function merge_power_pole_groups(mod_context, pole_groups)
...
function group_power_poles(mod_context, potential_pole_positions, consumer_positions)
    local iterations = 0;
...
function remove_consumer_from_pole_positions(mod_context, potential_pole_positions, consumer_position)
...
function find_next_power_pole(mod_context, potential_pole_positions, pole_groups, remaining_consumers)
...
function score_potential_pole(mod_context, pole, position, nearby_pole_position, score_to_beat)
...
function score_in_range_of_nearby_pole(mod_context, position, nearby_pole_position)
    local wire_range = mod_context.toolbox.power_pole.wire_range
...
function score_consumer_count(pole)
    local consumer_weight = 1;
    return pole.consumer_count * consumer_weight
end

score_adjacent_extractor = 0.55
score_adjacent_pipe = 0.55
max_adjacent_score = math.max(score_adjacent_extractor, score_adjacent_pipe) * 4

function score_adjacent_cells(mod_context, position)    
    local score = 0
...
function map_possible_consumer_power_pole_positions(mod_context, consumer_positions) 
...
function get_power_pole_bounds_for_consumer(mod_context, consumer_position)
